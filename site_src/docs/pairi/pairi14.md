...
## 例 2：坏形互比时，嫌二度受け更重的一边

第二例是坎张之间的比较。

当两边都不漂亮时，就要看谁的重叠更严重、谁更容易形成二度受け。原例里索子一侧对 `3s` 的依赖更别扭，因此切 `2s`。
...
但如果这样做会让整手受入减少、二度受け加重，那就不值。原例中应切 `3p`，把明显更差的重叠关系先拆掉，而不是为一杯口幻想买单。
