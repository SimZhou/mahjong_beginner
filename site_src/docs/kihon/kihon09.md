# 复合形（1）

三张复合形的种类其实不多，核心只有两类：

1. `对子 + 搭子`
2. `两嵌`

它们之所以重要，不是因为名字难，而是因为很多人一看到这类结构就急着“定成对子”或“定成搭子”，结果把受入白白切窄。

## 对子加搭子：不要太早定型

这类结构的特点是，搭子里有一张又重了一枚，于是除了原本的顺子受入，还多了“再重成刻子”的可能。

常见三种：

| 形状 | 理解 | 成面子牌 | 总受入 |
| --- | --- | --- | --- |
| <img src="../hai/man1.gif" width="24" height="34" /><img src="../hai/man2.gif" width="24" height="34" /><img src="../hai/man2.gif" width="24" height="34" /> | 边张 + 对子 | <img src="../hai/man2.gif" width="24" height="34" />、<img src="../hai/man3.gif" width="24" height="34" /> | 6 |
| <img src="../hai/pin6.gif" width="24" height="34" /><img src="../hai/pin8.gif" width="24" height="34" /><img src="../hai/pin8.gif" width="24" height="34" /> | 嵌张 + 对子 | <img src="../hai/pin7.gif" width="24" height="34" />、<img src="../hai/pin8.gif" width="24" height="34" /> | 6 |
| <img src="../hai/sou3.gif" width="24" height="34" /><img src="../hai/sou4.gif" width="24" height="34" /><img src="../hai/sou4.gif" width="24" height="34" /> | 两面 + 对子 | <img src="../hai/sou2.gif" width="24" height="34" />、<img src="../hai/sou4.gif" width="24" height="34" />、<img src="../hai/sou5.gif" width="24" height="34" /> | 10 |

这里的结论很直接：

1. 如果你把它硬定成对子，会损失顺子方向的受入
2. 如果你把它硬定成搭子，也会损失成刻子的受入
3. 所以基础打法通常是不急着定，先保留它的复合功能

## 例 1：别为了保对子去损失受入

例 1：<img src="../hai/man7.gif" width="24" height="34" /><img src="../hai/pin3.gif" width="24" height="34" /><img src="../hai/pin5.gif" width="24" height="34" /><img src="../hai/pin5.gif" width="24" height="34" /><img src="../hai/pin9.gif" width="24" height="34" /><img src="../hai/pin9.gif" width="24" height="34" /><img src="../hai/sou1.gif" width="24" height="34" /><img src="../hai/sou1.gif" width="24" height="34" /><img src="../hai/sou3.gif" width="24" height="34" /><img src="../hai/sou6.gif" width="24" height="34" /><img src="../hai/sou7.gif" width="24" height="34" /><img src="../hai/sou8.gif" width="24" height="34" /><img src="../hai/sou9.gif" width="24" height="34" /> 自摸<img src="../hai/sou2.gif" width="24" height="34" />

很多人会下意识切 <img src="../hai/pin3.gif" width="24" height="34" />，觉得这样能把对子留下来。

但这么做的问题是：

1. 直接抹掉 <img src="../hai/pin4.gif" width="24" height="34" /> 的受入
2. 也顺带取消了筒子部分后续变好的空间

标准思路是先切孤张更重的部分，而不是把复合形自己拆坏。

## 例 2：一向听更不能随便固定搭子

例 2：<img src="../hai/man2.gif" width="24" height="34" /><img src="../hai/man2.gif" width="24" height="34" /><img src="../hai/man7.gif" width="24" height="34" /><img src="../hai/man7.gif" width="24" height="34" /><img src="../hai/man8.gif" width="24" height="34" /><img src="../hai/pin4.gif" width="24" height="34" /><img src="../hai/pin5.gif" width="24" height="34" /><img src="../hai/pin6.gif" width="24" height="34" /><img src="../hai/pin6.gif" width="24" height="34" /><img src="../hai/pin8.gif" width="24" height="34" /><img src="../hai/sou4.gif" width="24" height="34" /><img src="../hai/sou5.gif" width="24" height="34" /><img src="../hai/sou6.gif" width="24" height="34" /> 自摸<img src="../hai/tyun.gif" width="24" height="34" />

这种牌从一向听往听牌推进时，受入已经很珍贵了。若先切 <img src="../hai/man7.gif" width="24" height="34" /> 去固定结构，原页的意思就是会把 `16 枚` 受入缩到 `12 枚`，损失很大。

结论：

**越接近听牌，越不能无理由把复合形硬拆成单一用途。**

## 两嵌：受入不差，但后期会发闷

第二类三张复合形是`两嵌`，也就是两个嵌张连在一起。

常见形如：

1. <img src="../hai/pin1.gif" width="24" height="34" /><img src="../hai/pin3.gif" width="24" height="34" /><img src="../hai/pin5.gif" width="24" height="34" />
2. <img src="../hai/pin2.gif" width="24" height="34" /><img src="../hai/pin4.gif" width="24" height="34" /><img src="../hai/pin6.gif" width="24" height="34" />
3. <img src="../hai/pin3.gif" width="24" height="34" /><img src="../hai/pin5.gif" width="24" height="34" /><img src="../hai/pin7.gif" width="24" height="34" />

这一类总共有 5 种，受入都是 `8 枚`，单看枚数其实和两面一样，属于不错的形。

但它和两面有本质差别：

1. 序盘很能工作
2. 临近听牌时不如两面自由
3. 一旦和其他结构挤在一起，容易让手牌发闷

## 例 3：两嵌可能和手役冲突

例 3：<img src="../hai/man2.gif" width="24" height="34" /><img src="../hai/man3.gif" width="24" height="34" /><img src="../hai/man4.gif" width="24" height="34" /><img src="../hai/man5.gif" width="24" height="34" /><img src="../hai/man7.gif" width="24" height="34" /><img src="../hai/man8.gif" width="24" height="34" /><img src="../hai/man9.gif" width="24" height="34" /><img src="../hai/pin5.gif" width="24" height="34" /><img src="../hai/pin7.gif" width="24" height="34" /><img src="../hai/pin9.gif" width="24" height="34" /><img src="../hai/sou3.gif" width="24" height="34" /><img src="../hai/sou3.gif" width="24" height="34" /><img src="../hai/sou5.gif" width="24" height="34" /> 自摸<img src="../hai/sou6.gif" width="24" height="34" />

这手若单看最快听牌，会倾向保留两嵌；但原页提醒的是，一旦这样走，可能会把一气通贯的路线直接丢掉，而且就算听牌，待张也未必理想。

所以两嵌虽然强，但不是无脑最优。

## 这一页的结论

1. 三张复合形重点只有两类：对子加搭子、两嵌
2. 对子加搭子不要太早固定成单一用途
3. 这种复合形的价值，正来自它同时保留多条进张路线
4. 两嵌在序盘是好形，但接近听牌时灵活度不如两面
5. 评价复合形不能只看当前受入，还要看后续形状和手役空间

---

原始日文页：<http://beginners.biz/kihon/kihon09.html>
