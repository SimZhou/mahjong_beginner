# 浮瓦理论

让我们思考一下 34 种阿苏牌各自具有的功能。

## 1. 基本牌

无论哪一种牌，变成对子或刻子的概率在 34 种牌里都是一样的。
不过，从Koko到Coates的变化很容易打（很容易出现在比赛中）
最靠近边缘的牌具有优势。

然而，Schunz 组合能力对于靠近中间的牌更有利。
说到人物牌，只有科茨可以制作。
假设面对面，

**<打脸能力>**

<img src="../hai/man3.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" /><img src="../hai/man4.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" /><img src="../hai/man5.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" /><img src="../hai/man6.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" /><img src="../hai/man7.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" />＞<img src="../hai/man2.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" /><img src="../hai/man8.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" />＞<img src="../hai/man1.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" /><img src="../hai/man9.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" />＞**字母牌**你可以这样对它们进行排名。 
图像是男人的，但剑和牌别针当然是一样的。 
请记住，内部牌更有价值。人物牌的价值简单来说就是可以当外套的牌价值高。Dub East/Dub South > 角色牌 > 宅男风格牌是。通过喊话就能吸引 1 名粉丝的胜牌价值相当高。
有很多东西比老头牌（19牌）更重要。
即使场上只有一张牌被切掉，它的价值也会大幅下降，
如果你看到第二个，那就没什么用了。在大多数情况下，如果您有一块，您会将其保留为安全牌。2. 磁贴交互即使几块牌是孤立的，三块牌的组合也被认为是不利的。特别地，在1、4、6和牌9的组合中，牌1和牌9分别几乎没有价值。→验收是～（与验收一张相同，不包括重叠）→验收是～（与验收一张相同，不包括重叠）仅当您抓到 4 张牌（例如 2356 和牌 2335）并获得两张牌时，14 中的 1 才有用。相隔三块牌的几块牌的组合称为条纹牌。
有一条规则，如果你持有带有条纹牌的牌，那么一张牌的价值就会减少。最值得注意的组合是 1-4、6-9、
同样，组合 2-5、3-6、4-7 和牌 5-8 在被接受时也会产生负面影响（可以形成棋盘的牌类型）。

如果持有固定牌的牌，则接受的牌张数会重复，结果，接受的牌张数会减少。手板砖
可以形成塔的 自摸 牌




































（比较）当你看表时就很明显了。在 的情况下，合并结果中的有效牌数量会减少，因为 ・ 的接受是重复的。如果它说2-5，接受1将是一种倾向，所以我不太高兴。您应该知道，持有 2-5 意味着留下 2 以便抽 3 并瞄准双方。这意味着如果 1 很薄并且
5 是
如果是红色，2的值就会减少。 5-8 中的 8 也是如此。


对于3-6和牌4-7，每张牌的价值都很高，所以
您不必从一开始就担心肌肉块。然而，像易象棋这样的例子，它是粘在一起的，用肌肉瓦来固定它是不利的。
例子
宝牌
 砍掉它就是一种损失。
如果同时保留 和牌 ，则会出现肌肉瓦片的负面影响。

如果没有条件，示例中的正确答案是 cut 。
理论

如果你持有固定牌的牌，有效牌会重叠，结果，接受牌的数量会减少。
特别是，数字为 14 时的 1 和牌数字为 69 时的 9 几乎不能作为面部候选，因此应尽快将其删除。


最后，我们来谈谈相隔四个位置的组合。 
有 5 个选项：1-5、2-6、3-7、4-8 和牌 5-9。
过去，这些组合很受重视，因为如果拉动中间的瓦片，就可以做出两铺。
例子

与自摸成为良谷
但这是一种毫无意义甚至有害的思维方式。
 即使您没有，我们仍然可以接受您。
例如，与 相比，它少接受一种类型。
如果你从 切入，你唯一会失败的时候是当你抽签的时候。
 没有特殊意义。
虽然四个相隔的组合可以形成两铺，
这并不一定意味着它是有利的。
综上所述，根本不用担心过梁书。
