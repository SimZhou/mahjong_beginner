...
图像是男人的，但剑和牌牌别针当然是一样的。 
请记住，内部瓷砖更有价值。人物瓷砖的价值简单来说就是可以当外套的瓷砖价值高。Dub East/Dub South > 角色瓷砖 > 宅男风格瓷砖是。通过喊话就能吸引 1 名粉丝的胜牌价值相当高。
有很多东西比老头牌（19牌）更重要。
即使场上只有一张牌被切掉，它的价值也会大幅下降，
如果你看到第二个，那就没什么用了。在大多数情况下，如果您有一块，您会将其保留为安全瓷砖。2. 磁贴交互即使几块瓷砖是孤立的，三块瓷砖的组合也被认为是不利的。特别地，在1、4、6和牌牌9的组合中，图块1和牌牌9分别几乎没有价值。→验收是～（与验收一张相同，不包括重叠）→验收是～（与验收一张相同，不包括重叠）仅当您抓到 4 张牌（例如 2356 和牌牌 2335）并获得两张牌时，14 中的 1 才有用。相隔三块瓷砖的几块瓷砖的组合称为条纹瓷砖。
有一条规则，如果你持有带有条纹牌的牌，那么一张牌的价值就会减少。最值得注意的组合是 1-4、6-9、
同样，组合 2-5、3-6、4-7 和牌牌 5-8 在被接受时也会产生负面影响（可以形成棋盘的瓷砖类型）。
...
对于3-6和牌牌4-7，每张牌的价值都很高，所以
您不必从一开始就担心肌肉块。然而，像易象棋这样的例子，它是粘在一起的，用肌肉瓦来固定它是不利的。
例子
宝牌
 砍掉它就是一种损失。
如果同时保留 和牌牌 ，则会出现肌肉瓦片的负面影响。

如果没有条件，示例中的正确答案是 cut 。
理论

如果你持有固定牌的牌，有效牌会重叠，结果，接受牌的数量会减少。
特别是，数字为 14 时的 1 和牌牌数字为 69 时的 9 几乎不能作为面部候选，因此应尽快将其删除。


最后，我们来谈谈相隔四个位置的组合。 
有 5 个选项：1-5、2-6、3-7、4-8 和牌牌 5-9。
