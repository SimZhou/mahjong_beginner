...
⇒<img src="../hai/sou2.gif" style="display:inline;vertical-align:middle;margin:0 1px;"width="24" height="34"/>剪切
...
例1是最简单的筑木一象棋。
如果是简单组合，则按照牌规则，牌将按以下优先顺序保留：3-7 > 2.8 > 1.9。
所以这里切割是没有问题的。
然而，即使是低效的裁切，接受的张数也超过了30张。
考虑到双面x 2易象棋只有4种16枚，
我想你也能理解易想奇能以多快的速度到达奇牌。
然而，在如示例1那样简单粘合的情况下
被认为是搭子良好的牌的数量很少，即使是切割的牌也只有16块。

对于粘贴搭子，您应该关注形成良好搭子的牌数量。
我相信30件就足够被柒牌录取了。
即使30张牌变成31张牌，也几乎没有什么效果。
演奏屋久时可以坚持屋久的搭子，或者屋久的类型。
 你可以认为这是一个难得的机会。
...
搭子良好的牌数量相反：“Kiri = Kiri > Kiri”。
因此，切割是这里的正确答案。
 比较“kiri”和牌“kiri”是因为接受的张数也不同，而且抽签也有三面。
示例 2 是所有“3”块的组合。
请记住，“如果您拥有相同数量的牌，则多个牌的组合总体上比单个牌更强。”
...
如果您最终得到的是粘性形式的 Gugata 牌，那么不使用该牌是一个不错的选择。
例如，在这种情况下，
自摸
　
不管怎么样，我对这样的单鞭七是不满意的。
由于月石牌数量较多，即使破坏了月石牌，也能期待快速恢复。
所以，我们就砍掉自摸，从下一轮开始接起自摸吧。
