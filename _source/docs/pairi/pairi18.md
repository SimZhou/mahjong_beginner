# イー向听の牌理(6)

くっつき形イー向听は、

(一)广泛接受

(2) 容易发生古加塔皮卡里病

它具有以下特点。

示例 1 <img src="../hai/man4.gif" style="display:inline;vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/pin1.gif" style="display:inline;vertical-align:middle;margin:0 1px;" width="24" height="34"/><img src="../hai/pin2.gif" style="display:inline;vertical-align:middle;margin:0 1px;" width="24" height="34"/><img src="../hai/pin3.gif" style="display:inline;vertical-align:middle;margin:0 1px;" width="24" height="34"/><img src="../hai/pin5.gif" style="display:inline;vertical-align:middle;margin:0 1px;" width="24" height="34"/><img src="../hai/pin7.gif" style="display:inline;vertical-align:middle;margin:0 1px;" width="24" height="34"/><img src="../hai/pin8.gif" style="display:inline;vertical-align:middle;margin:0 1px;" width="24" height="34"/><img src="../hai/pin9.gif" style="display:inline;vertical-align:middle;margin:0 1px;" width="24" height="34"/><img src="../hai/sou1.gif" style="display:inline;vertical-align:middle;margin:0 1px;" width="24" height="34"/><img src="../hai/sou2.gif" style="display:inline;vertical-align:middle;margin:0 1px;" width="24" height="34"/><img src="../hai/sou2.gif" style="display:inline;vertical-align:middle;margin:0 1px;" width="24" height="34"/><img src="../hai/sha.gif" style="display:inline;vertical-align:middle;margin:0 1px;" width="24" height="34"/><img src="../hai/sha.gif" style="display:inline;vertical-align:middle;margin:0 1px;" width="24" height="34"/> Tsumo<img src="../hai/sou3.gif" style="display:inline;vertical-align:middle;margin:0 1px;"宽度=“24”高度=“34”/>

⇒<img src="../hai/sou2.gif" style="display:inline;vertical-align:middle;margin:0 1px;"宽度=“24”高度=“34”/>剪切

愉快的机会
接受的张数


切

31张（11张）


切

33 件（11 件）


切

38张（16张）


例1是最简单的筑木一象棋。
如果是简单组合，则按照图块规则，图块将按以下优先顺序保留：3-7 > 2.8 > 1.9。
所以这里切割是没有问题的。
然而，即使是低效的裁切，接受的张数也超过了30张。
考虑到双面x 2易象棋只有4种16枚，
我想你也能理解易想奇能以多快的速度到达奇牌。
然而，在如实施例1那样简单粘合的情况下
被认为是形状良好的瓷砖的数量很少，即使是切割的瓷砖也只有16块。

对于粘贴形状，您应该关注形成良好形状的瓷砖数量。
我相信30件就足够被柒牌录取了。
即使30张牌变成31张牌，也几乎没有什么效果。
演奏屋久时可以坚持屋久的形状，或者屋久的类型。
        你可以认为这是一个难得的机会。

实施例2 津莫
⇒ 剪切
例2是一个典型例子。获胜机会如下。



 
愉快的机会
接受的张数


切

43 件（16 件）


切

37张（22张）


切

40张（22张）



如果原则是机会优先的话，那是不行的，但是
形状良好的瓷砖数量相反：“Kiri = Kiri > Kiri”。
因此，切割是这里的正确答案。
 比较“kiri”和“kiri”是因为接受的张数也不同，而且抽签也有三面。
示例 2 是所有“3”块的组合。
请记住，“如果您拥有相同数量的图块，则多个图块的组合总体上比单个图块更强。”


实施例3 津莫
⇒ 剪切



 
愉快的机会
接受的张数


切

50张（26张）


切

46张（29张）


切

38张（24张）



剑的连续形态是最好的四连体形态，所以我一定会保留它。
剪裁有点精致，不过46片就足够接受了。
我们建议切割 29 块形状良好的瓷砖。 
两侧容易卷曲的凸出形状极具魅力。
我已经解释了连续形式的强度，所以我不会列出所有的模式。
理论/总结

因为Tsukuki E-Kou的接受张数足够宽，
形状良好的瓷砖数量是最重要的因素。

如果您最终得到的是粘性形式的 Gugata 瓷砖，那么不使用该瓷砖是一个不错的选择。
例如，在这种情况下，
津莫
　
不管怎么样，我对这样的单鞭七是不满意的。
由于月石牌数量较多，即使破坏了月石牌，也能期待快速恢复。
所以，我们就砍掉Tsumo，从下一轮开始接起Tsumo吧。

