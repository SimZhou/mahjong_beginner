...
这是因为即使接受度下降，如果我们考虑牌的数量，达到牌数量的速度可能会逆转。

为了使结论尽可能清晰，我们从门森效率的角度来讨论。

作为初始牌系统的示例，我们以一只手上有多个浮动牌为例。
...
这里<img src="../hai/man7.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" /><img src="../hai/pin2.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" /><img src="../hai/sou1.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" /><img src="../hai/nan.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" />它是一个孤立的牌，不会击中 Menko、Toko 或 Koko 中的任何一个。
更不用说最不实用的牌了。<img src="../hai/nan.gif" style="display:inline; vertical-align:middle; margin:0 1px; width:24px; height:34px;" />是。
...
显然，能够制作 Schunz 的几块牌更有价值。因此，这里只能切割牌。
虽然单独切掉也没有多大损失。
”
...
换句话说，除了像字母块一样堆叠起来之外，它们没有任何用处。
如果是这样的话，这块牌不是更有价值吗？因为如果你在下一轮快速恢复，你可以扔掉这块牌，并且即使它们重叠，你也可以稍后扔掉 2 个作为安全牌？ 
理论/总结
孤立的角色牌是最不可能引起尴尬的部分，
如果有无用的风瓦，砍掉它是绝对正确的。
然而，如果有明显不必要的牌，
考虑到后期的防御，还有一个办法就是砍几块牌。

示例3
津茂⇒切
单独的牌有四种类型，因此切掉价值最低的一种。
牌数量之间的关系是 1.9 < 2.8 < 3-7，所以
 是顶部不必要的牌。
你剪哪一个并不重要，但是
因为有条纹牌，所以就算两个都抽了也不会亏。
 如果你砍了，如果你减去了两张脸，你就会失去一张脸。 
因此，从 中剪切是标准做法。
...
孤立的牌是
 3种。
即使是胜牌，如果是 Menzen 的恢复效率的话。
然而，如果你有这个级别的困难动作，
您不能忽视使用方形牌所带来的速度提升。
这是一手有其他不必要的牌的牌。屋久牌没有被切割。

哈

没有牌可以跟随，所以我离开了。所以剩下的就是引脚了。
每块肌肉瓦片
...
如果你剪
如果你抽到“但没有抽到”，你就会输，所以这是价值最低的牌。



示例5
自摸 ⇒ Cut 如果您要比较相同数量的牌，则应该保留连续形式。
即使是相同的旧头牌（1、9牌），连续的一块也比单独的旧头牌要好。
 接受和牌积极的改变都很棒。 （第一章 4页复合表格）
另外，在正常面手上，孤立的老牌总是不如贝子和牌东光。
（当然，不包括涉及角色的情况，比如混色）
...
示例6
 ...
 这是一个比较。
我刚拿到积分
您还可以通过切割然后拉动来制作杠陈tōshi。
但是
 
	切割后
	
	如果把它拔出来，就彻底损失了。 
 将孤立的瓦片作为肌肉瓦片持有仍然是一个缺点。
 （第二章浮瓦原理）
...
切割完全是一个糟糕的举动。 
雀士们都知道，7 块牌比 8 块牌好。 
这样的话，既然有E-Peco，那么作用上就会有很大的损失。 
出于某种原因，有些人不愿意以这种形式持有它......

示例7
津茂⇒切
处理暗刻四张连牌时需要注意的是，手中有两张或更多牌时。
由于我们不再需要雀头，我们必须想象从这种连续的形式创建一个两张脸的孩子。如果是这样，可见要挽回面子是相当困难的。
