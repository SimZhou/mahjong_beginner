...
如果是这样的话，这块瓷砖不是更有价值吗？因为如果你在下一轮快速恢复，你可以扔掉这块瓷砖，并且即使它们重叠，你也可以稍后扔掉 2 个作为安全瓷砖？ 
理论/总结
孤立的角色图块是最不可能引起尴尬的部分，
如果有无用的风瓦，砍掉它是绝对正确的。
然而，如果有明显不必要的瓷砖，
考虑到后期的防御，还有一个办法就是砍几块瓷砖。

实施例3
津茂⇒切
单独的瓷砖有四种类型，因此切掉价值最低的一种。
瓷砖数量之间的关系是 1.9 < 2.8 < 3-7，所以
 是顶部不必要的瓷砖。
你剪哪一个并不重要，但是
...
然而，如果你有这个级别的困难动作，
您不能忽视使用方形瓷砖所带来的速度提升。
这是一手有其他不必要的牌的牌。屋久瓷砖没有被切割。

哈

没有瓷砖可以跟随，所以我离开了。所以剩下的就是引脚了。
每块肌肉瓦片
...
实施例5
Tsumo ⇒ Cut 如果您要比较相同数量的瓷砖，则应该保留连续形式。
即使是相同的旧头牌（1、9牌），连续的一块也比单独的旧头牌要好。
  接受和积极的改变都很棒。 （第一章 4页复合表格）
另外，在正常面手上，孤立的老牌总是不如贝子和东光。
（当然，不包括涉及角色的情况，比如混色）
...
由于我们不再需要麻雀头，我们必须想象从这种连续的形式创建一个两张脸的孩子。如果是这样，可见要挽回面子是相当困难的。
