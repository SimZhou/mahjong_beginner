# 站立还是库罗？ (2)

是否卷土重来

第二次，我们将考虑 是充分形式的情况。

听が好是指容易生气的人。

双面和多边纸张的纸张数量较多，因此您可以期待它们的效果良好。

容易输的手牌，比如沙波带信、单马带信。

**例１**
<img src="../hai/pin1.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/pin1.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/pin3.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/pin4.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/pin5.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/sou2.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/sou2.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/sou2.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/sou7.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/sou8.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/sou9.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/pei.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/><img src="../hai/pei.gif" style="display:inline; vertical-align:middle; margin:0 1px;" width="24" height="34"/>

示例 1 为 <img src="../hai/pin1.gif" style="display:inline;vertical-align:middle; margin:0 1px;" width="24" height="34"/> 和 <img src="../hai/pei.gif" style="display:inline;vertical-align:middle; margin:0 1px;" width="24" height="34"/> 虽然是shabbos，但是两块牌都很容易上手，可以说是不错的。
即使只有4张，也比劣质的双面更容易卡住。

现在，让我们举一个例子，你玩了一手 5,200 或更少的牌，双方的 zi 都很好。

结论是“一切都应该在平坦的场地上恢复”**。


实施例2
津摩宝藏瓷砖
过去的理论是“只有和平才是和平”。普遍的观点是，没有必要通过拿出一根1000点的站立棒，让价值1000点的棋步上升到2000点。
然而，使用“太刀平和”，你可以预期平均超过3,000点阿加里点，所以知道即使你使用太刀棍也不会损失。
只有和平才能使你站直。
是目前的理论。
例2也
 正确答案是立即站直。

实施例3

宝藏瓷砖

还有一些手牌可以让您一次瞄准不同类型的卡片，例如三种颜色和一个字母。
示例 3 就是一个这样的例子。
但只有一种，抽中的概率较低。
通常，Agari 瓷砖会在易手之前出现。
在示例3中，使用Yamiten三种颜色是一个非常糟糕的选择。
一个直立的粉丝，一击出秘宝牌，这就更加确定了。
实施例4
　
宝藏瓷砖

示例4是一个稍微改变的例子，改变了234和345。
不过，这对于手瓦里甘利来说仍然是一个劣势。
即使换成三色，除非获得高卷，否则也做不到三色。如果您立即站起来参加比赛，则预期得分会高于等待不确定的 2 名球迷更换的情况。
尤其是平娃-宝藏1、断拳9-平娃这样的2扇招式，据说在单人站立时最能有效提高分数，而且无论有多少招式，立即站立整体效果更好。

示例5 宝藏牌
示例5有很多变化，
 有5种类型。
在这种极端的情况下，我会考虑Kurinten，但可以说是很少见的情况。
如果您立即从示例 5 中恢复过来，则完全没有问题。
过去，阿修经常使用“最终形态”和“最高形态”这两个词，
这是一个毫无意义的概念。
在示例5中

但我会在这件事发生之前就上去，哈哈
无需担心最终形式或最佳形式。
如果这个举动能让你重新站起来，那么你就应该重新站起来。
理论

好的笔排的基础是站直。
忽略任何更改！

